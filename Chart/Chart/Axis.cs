...
namespace xuzhenzhen.com.chart
{
	...
	public class Axis : System.ICloneable
	{
		...
		private void Init()
		{
			...
			this.TickTextFont = new Font( FontFamily.GenericSerif, 10, FontStyle.Regular);
			this.LabelFont = new Font( FontFamily.GenericSerif, 10, FontStyle.Regular);
			...
			bool clip )
		{
			...
			if ( clip )
			{
				if ( WorldMin < WorldMax )
				{
					if ( coord > WorldMax )
					{
						return new PointF(_physicalMax.X,_physicalMax.Y);
					}
					if ( coord < WorldMin )
					{
                        return new PointF(_physicalMin.X, _physicalMin.Y);
					}
				}
				else
				{
			...
			Point axisPhysicalMax )
		{
			...
			if ( Label != "" )
			{
				...
				extraOffsetAmount += 4.0f; // empirically determed - text was too close to axis before this.
                ...
                Font font = CreateRotatedFont((int)theta, g);
				SizeF labelSize = g.MeasureString( Label,labelFontScaled_);
                if (theta > 20 || theta < -20)
                {
                    drawFormat_ = new StringFormat(StringFormatFlags.NoWrap | StringFormatFlags.NoClip);
                    float x0 = offset.X + average.X - labelSize.Height-7;
                    float y0 = offset.Y + average.Y-labelSize.Width/2;
                    g.DrawString(Label, font, labelBrush_,x0, y0,drawFormat_);
                    return new Rectangle((int)x0, (int)y0, (int)labelSize.Height, (int)labelSize.Width);
                }
                else
                {
                    float x0 = offset.X + average.X;
                    float y0 = offset.Y + average.Y - labelSize.Height / 2;
                    g.DrawString(Label,font,labelBrush_,x0, y0,drawFormat_);
                    return new Rectangle((int)x0, (int)y0, (int)labelSize.Width, (int)labelSize.Height);
                }
