				...
				if ( (!double.IsNaN (point.Open)) && (!double.IsNaN(point.High)) && (!double.IsNaN (point.Low)) && (!double.IsNaN(point.Close)) )
				{
					...
					if (this.Style == Styles.Stick)
					{
						...
						*/
                        ...
                        g.DrawLine(p, (int)(xPos + offset), yPos1, (int)(xPos + offset), yPos2);
                        g.DrawLine(p, (int)(xPos - addAmount + offset), yPos3, (int)(xPos + offset), yPos3);
                        g.DrawLine(p, (int)(xPos + offset), yPos4, (int)(xPos + addAmount + offset), yPos4);
					}

					else if (this.Style == Styles.Filled)
					{
                        g.DrawLine(p, xPos + (int)offset, yPos1, xPos + (int)offset, yPos2);
						if (yPos3 > yPos4)
						{
                            g.FillRectangle(bullishBrush, xPos - (int)addAmount + (int)offset, yPos4, (int)stickWidth, yPos3 - yPos4);
                            g.DrawRectangle(p, xPos - (int)addAmount + (int)offset, yPos4, (int)stickWidth, yPos3 - yPos4);
						}
						else if (yPos3 < yPos4)
						{
                            g.FillRectangle(bearishBrush, xPos - (int)addAmount + (int)offset, yPos3, (int)stickWidth, yPos4 - yPos3);
                            g.DrawRectangle(p, xPos - (int)addAmount + (int)offset, yPos3, (int)stickWidth, yPos4 - yPos3);
						}
						else
						{
                            g.DrawLine(p, xPos - (int)addAmount + (int)offset, yPos3, xPos - (int)addAmount + (int)stickWidth + (int)offset, yPos3);
						}
