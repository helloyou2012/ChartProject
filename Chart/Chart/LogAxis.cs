	...
	public class LogAxis : Axis
	{
			...
			out object boundingBox )
		{
			...
			if (largeTickPositions.Count > 0)
			{
				for (int i=0; i<largeTickPositions.Count; ++i)
				{
					...
					label.AppendFormat(null,this.NumberFormat, (double)largeTickPositions[i]);
			...
			bool clip )
		{
			...
			if (clip)
			{
				if (coord > WorldMax)
				{
					return new PointF(physicalMax.X,physicalMax.Y);
				}
				if (coord < WorldMin)
				{
					return new PointF(physicalMin.X,physicalMin.Y);
				}
			}
